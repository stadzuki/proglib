...
procedure TcontentPage.LoadComment(var CommentAdd, CommentAthor, CommentText, CoomentLike, CommentDis:boolean);
var math:real;
...
begin
  ...
  if (CommentAthor = true) or (CommentText = true) or (CoomentLike = true) or (CommentDis = true) then
  begin
...
var QueryLikeCount, id, qCommentID:integer;
getID:TImage absolute sender;
begin
  id:=GetID.Tag;
  qCommentID := db.QueryPage['id'];
  db.QueryPage.SQL.Text := 'SELECT * FROM likes WHERE id_comment = :id_comment AND id_page = :id_page AND id_laker = :id_laker';
