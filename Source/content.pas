...
procedure getContentHeight(var count:integer; cH:integer; cTo:integer; cDo:integer);//Получение высоты контента
...
begin
  ...
  begin
    ...
    iAdmin.Caption := 'Добавить контент';
    iAdmin.Left := 628;
...
var math:real;
sender, i, resultMath, getType:integer;
begin
...
//    for i := 1 to db.QueryPage.RecordCount do
    ...
    begin
      ...
      db.QueryComment.SQL.Text := 'SELECT * FROM likes WHERE id_comment = :id_comment AND id_liker = :id_liker';
      ...
      begin
        ...
        if not db.QueryComment.IsEmpty then pLikeComment[i].Picture.LoadFromFile('images/like-enabled.png')
        else pLikeComment[i].Picture.LoadFromFile('images/like.png');
      end;
      ...
      begin
        ...
        if not db.QueryComment.IsEmpty and db.QueryComment['type'] = -1 then pDislikeComment[i].Picture.LoadFromFile('images/dislike-enabled.png')
        else pDislikeComment[i].Picture.LoadFromFile('images/dislike.png');
      end;
...
//  end;
  ...
  begin
    db.QueryPage.SQL.Clear;
    db.QueryPage.SQL.Add('INSERT INTO likes (id_liker, id_page, id_comment) VALUES (:id_liker, :id_page, :id_comment);');
    db.QueryPage.ParamByName('id_liker').AsInteger := QueryUserID;
    db.QueryPage.ParamByName('id_page').AsInteger := PageID;
    db.QueryPage.ParamByName('id_comment').AsInteger := qCommentID;
//    db.QueryPage.ParamByName('type').AsInteger := 1;
