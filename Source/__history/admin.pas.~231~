...
procedure Tadmin_control.CreateParams(var Params: TCreateParams);//Создание иконки на панели задачь
begin
  inherited;
...
begin
  if (iTitle.Text = '') or (iTitle.Text = 'Укажите тег') then begin showmessage('Укажите тег'); iTitle.Text := 'Укажите тег'; exit; end;
  if (iDesc.Lines.Text = '') or (iDesc.Lines.Text = 'Укажите описание тега') then begin showmessage('Укажите описание тега'); iDesc.Lines.Text := 'Укажите описание тега'; exit; end;
...
begin
  if (length(c_description.Lines.Text) > 0 )  AND (not ansicomparetext(c_description.Lines.Text, 'Введите заголовк') = 0) then c_description.Lines.Text := c_description.Lines.Text
  else c_description.Lines.Text := '';
end;

procedure Tadmin_control.c_titleClick(Sender: TObject);
begin
  if (length(c_title.Lines.Text) > 0 )  AND (not ansicomparetext(c_title.Lines.Text, 'Введите заголовк') = 0) then c_title.Lines.Text := c_title.Lines.Text
  else c_title.Lines.Text := '';
end;

procedure Tadmin_control.linkClick(Sender: TObject);
begin
  if (length(link.Text) > 0 )  AND (not ansicomparetext(link.Text, 'Ссылка на скачивание') = 0) then link.Text := link.Text
  else link.Text := '';
end;
